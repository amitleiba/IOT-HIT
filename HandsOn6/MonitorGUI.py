...
RELAY_COMMAND_TOPIC = "pr/home/9147953/sts"
...
class Mqtt_client:
    ...
    def on_message(self, client, userdata, msg):
        ...
        try:
            if len(parts) >= 4 and parts[0] == "Temperature:":
                temp = float(parts[1])

                # Alarm Threshold
                if temp > 30.0:
                    print(f"ALARM! High temperature: {temp}°C")

                    # 1. Control the RELAY automatically
                    on_payload = {"type": "set_state", "action": "set_value", "addr": 0, "cname": "ONOFF", "value": 1}
                    self.publish_to(RELAY_COMMAND_TOPIC, json.dumps(on_payload))

                    # 2. Trigger UI Popup
                    mainwin.alarm_signal.emit(temp)

        except Exception as e:
            print("Error in custom logic:", e)
    ...
    def connect_to(self):
        ...
        self.client = mqtt.Client(mqtt.CallbackAPIVersion.VERSION1, client_id=self.clientName, clean_session=True)
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        self.alarm_signal.connect(self.show_alarm_popup)

    def show_alarm_popup(self, temp):
        msg = QMessageBox()
        msg.setIcon(QMessageBox.Critical)
        msg.setWindowTitle("TEMPERATURE ALARM")
        msg.setText(f"Temperature reached {temp}°C!\nCooling system (RELAY) has been activated automatically.")
        msg.exec_()
